...
const prepareAvatar = async (
  ...
  try {
    ...
    if (
      ...
      if (potentialAvatar.indexOf("eip155:1/erc721") !== -1) { stub = potentialAvatar.split("eip155:1/erc721:")[1] }
...
const useENSContext = ({ ensAddress }) => {
  ...
  useEffect(() => {
    ...
    const main = async () => {
      setEnsError(null)
      const resolver = await context.web3Provider.getResolver(currentEnsAddress)

      if (resolver === null) { throw new Error('No resolver found for "' + currentEnsAddress + '"') }
