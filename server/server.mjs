...
export class Server {
	...
	async start(startExpress = true) {
		...
		this.ipfs = create({
			port: 5050,
			url: this.config.magicMirror.ipfsEndpoint || "https://ipfs.io/ipfs/",
		});
					...
					.map(
						(route) =>
							`<li><a href="${route.path}">${route.path}</a> ${
								route?.settings?.requireLogin ? "(Requires Login)" : ""
			...
			files.map(async (file) => {
				...
				let _route = async (method, res, req) => {
					...
					try {
						...
						console.log(`Error in ${method.toUpperCase()} route: ` + path);
						console.error(error);
